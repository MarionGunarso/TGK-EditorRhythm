...
public class ButtonScript : MonoBehaviour {
	...
	public void LoadListSong()
	{
		...
		//check if file list song is exist then open and add name to it, if not create new nameData, add name and then create file
		if(File.Exists(Application.persistentDataPath+"/listSongs.dat"))
		//if(File.Exists(Application.dataPath+"/Data/listSongs.dat"))
		{
			BinaryFormatter bf = new BinaryFormatter();
			FileStream file1 = File.Open(Application.persistentDataPath +"/listSongs.dat",FileMode.Open);
			//FileStream file1 = File.Open(Application.dataPath+"/Data/listSongs.dat",FileMode.Open);
			if(file1 != null)
			{
				Debug.Log("NOT NULL");
	...
	IEnumerator CoroutineActivate()
	{
		...
		//buat indikator nyelesein noteny
		playMode.GetComponent<PlayModeScript>().finishing=true;
		...
		{
			//Debug.Log("activate: "+a);
			//if(activableObject.name!="Spawner")
			//{
				activableObject.SetActive(false);
			//}
		...
		//reset
		playMode.GetComponent<PlayModeScript>().finishing=false;
		//playMode.SetActive(true);
